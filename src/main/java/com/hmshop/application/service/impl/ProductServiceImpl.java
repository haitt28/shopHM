...
public class ProductServiceImpl implements ProductService {
    ...
    public List<ProductInfoDTO> getListBestSellProducts() {
        ...
        return checkPublicCoupon(productInfoDTOS);
    }
    ...
    public DetailProductInfoDTO getDetailProductById(String id) {
        ...
        Coupon Coupon = couponService.checkPublicCoupon();
        if (Coupon != null) {
            dto.setCouponCode(Coupon.getCouponCode());
            dto.setCouponPrice(couponService.calculateCouponPrice(dto.getPrice(), Coupon));
        } else {
            dto.setCouponCode("");
        }
        return dto;
    ...
    public List<ProductInfoDTO> getRelatedProducts(String id) {
        ...
        return checkPublicCoupon(products);
    }
...
//        Optional<ProductSize> productSizeOld = productSizeRepository.getProductSizeBySize(createSizeCountRequest.getSize(),createSizeCountRequest.getProductId());
    ...
    @Override
    public List<ProductInfoDTO> checkPublicCoupon(List<ProductInfoDTO> products) {
        //Kiểm tra có khuyến mại
        Coupon Coupon = couponService.checkPublicCoupon();
        if (Coupon != null) {
            ...
            for (ProductInfoDTO product : products) {
                ...
                if (Coupon.getDiscountType() == DISCOUNT_PERCENT) {
                    long tmp = product.getPrice() * Coupon.getDiscountValue() / 100;
                    if (tmp < discountValue) {
                        discountValue = tmp;
                    }
                }
                long CouponPrice = product.getPrice() - discountValue;
                if (CouponPrice > 0) {
                    product.setCouponPrice(CouponPrice);
                } else {
                    product.setCouponPrice(0);
                }
            }
        }
    ...
    public PageableDTO filterProduct(FilterProductRequest req) {
        ...
        return new PageableDTO(checkPublicCoupon(products), totalPages, req.getPage());
    ...
    public PageableDTO searchProductByKeyword(String keyword, Integer page) {
        ...
        return new PageableDTO(checkPublicCoupon(products), totalPages, page);
    }
